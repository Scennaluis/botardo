        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          if (m.chat in global.DATABASE._data.chats || m.sender in global.DATABASE._data.users) {
            ...
            if (!['unbanuser.js', 'inv.js', 'link.js', 'creator.js', 'profile.js'].includes(name) && user && user.banned && !isROwner) {
              if (!opts['msgifbanned']) m.reply(`*鈿狅笍 ESTAS BANEADO 鈿狅笍* ${user.bannedReason ? `\n*Motivo:* *${user.bannedReason}*` : ''}
...
`.trim())
          ...
          if (!isPrems && plugin.limit && global.DATABASE._data.users[m.sender].limit < plugin.limit * 1) {
            this.reply(m.chat, `*Tu limite para usar este comando terminó*`, m)
...
鈹佲攣鈹佲攣猬�  饾樇饾檳饾檹饾檮 饾樋饾檧饾檱饾檧饾檹饾檧  猬ｂ攣鈹佲攣鈹�
*鈻� Nombre:* @${m.participant.split`@`[0]}
*鈻� Enviando el mensaje..*
*鈻� Para desactivar esta funci贸n envie el comando:* #disable delete
鈹佲攣鈹佲攣猬�  饾樇饾檳饾檹饾檮 饾樋饾檧饾檱饾檧饾檹饾檧  猬ｂ攣鈹佲攣鈹�
`.trim(), m.message, {
      contextInfo: {
  ...
  async onCall(json) {
    ...
    await this.sendMessage(from, '*[ 鈿� 锔廬 锔廚O LLAMAR AL BOT POR FAVOR, NO QUEREMOS BLOQUEARL@, EVITENOS LA PENA DE HACERLO [ 鈿� 锔廬*\n\n*鉂桽I ESTE INCIDENTE SE REPITE USTED SERA BLOQUEAD@鉂�*', MessageType.extendedText)
  }
}

global.dfail = (type, m, conn) => {
  let msg = {
    rowner: '*[ 鈿� 锔廬 Este comando solo puede ser utilizado por el propietario/owner del Bot*',
    owner: '*[ 鈿� 锔廬 Este comando solo puede ser utilizado por sub Bots y el propietario*',
    mods: '*[ 鈿� 锔廬 Este comando solo puede ser utilizado por moderadores y el propietario/owner del Bot*',
    premium: '*[ 鈿� 锔廬 Este comando es solo para miembros premium*',
    group: '*[ 鈿� 锔廬 Este comando solo se puede usar en grupos*',
    private: '*[ 鈿� 锔廬 Este comando solo se puede usar en el chat privado del numero del Bot*',
    admin: '*[ 鈿� 锔廬锔� Este comando es solo para administradores del grupo*',
    botAdmin: '*[ 鈿� 锔廬 Ascender el Bot a admin para usar este comando*',
    unreg: '*[ 鈿� 锔廬锔� Reg铆strese para utilizar esta funci贸n escribiendo:*\n\n*#registrar nombre.edad*\n\n*Ejemplo:*\n*#registrar zorrita.18*\n\n*Este registro se solicita con el fin de que el Bot no se sature*\n\nNota: No escriba los "*" ni los "_"'
  }[type]
